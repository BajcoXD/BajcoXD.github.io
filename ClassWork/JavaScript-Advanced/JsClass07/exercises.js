...
fetch(
  ...
  .then((students) => {
    ...
    const studentsWithAvgHigherThan3 = students.filter(
      (s) => s.averageGrade > 3
    );
    ...
    const maleFromSkopjeFullNames = students
    ...
    const maleStartingWithBAvgGrade = students.filter(
      (s) =>
        s.gender === "Male" && s.firstName.startsWith("B") && s.averageGrade > 2
    );
    ...
    const googleMailCount = students.filter(
      (s) => s.email.includes("@gmail") || s.email.includes("@google")
    ).length;
    console.log(googleMailCount); // 15

    // The average age of female students living in Skopje
    const femaleStudentsInSkopje = students.filter(
      (s) => s.gender === "Female" && s.city === "Skopje"
    );
    const ageSum = femaleStudentsInSkopje.reduce(
